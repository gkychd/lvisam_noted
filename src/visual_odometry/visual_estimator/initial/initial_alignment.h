...
public:
    ...
    n(n_in)
    {
        q_lidar_to_cam = tf::Quaternion(0, 1, 0, 0); // rotate orientation // mark: camera - lidar （x,y,z,w）
    ...
    vector<float> getOdometry(deque<nav_msgs::Odometry>& odomQueue, double img_time)
    {
        ...
        tf::quaternionMsgToTF(odomCur.pose.pose.orientation, q_odom_lidar);//Rwl'也就是Rwl
