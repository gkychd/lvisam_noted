...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &image, double td)
{
    ...
    {
        FeaturePerFrame f_per_fra(id_pts.second[0].second, td);//id_pts.second[0]表示(相机序号，特征点属性); id_pts.second[0].second为特征点的属性（在当前图像帧下
                                                                                                                                                                                        //特征点的属性，其在不同图像帧下不同）
        ...
        //注意：这里的feature在每调用addFeatureCheckParallax函数时不会更新，因此新的图像传入时，根据特征点索引可以找到多次跟踪的点
        // 没有找到，是新的特征点
        if (it == feature.end())
        {
        ...
        else if (it->feature_id == feature_id) // 找到了，就是跟踪上的点
        {
            ...
            // 以下部分是新的内容 如果有雷达的深度，则更新特征点管理器中特征点的深度
