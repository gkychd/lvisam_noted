...
public:
    ...
    void performLoopClosure(const std_msgs::Float64MultiArray& loopMsg)
    {
        ...
        pcl::PointCloud<PointType>::Ptr prevKeyframeCloud(new pcl::PointCloud<PointType>());
        {
            ...
            //找到候选帧前后各25个关键帧的点云(世界系下)组成 prevKeyframeCloud
        ...
        Eigen::Affine3f pose_diff_t; // serves as initial guess
        {
            ...
            // t_diff = pose_pre - pose_cur    twp - twc
        ...
        // 相当于将W系的点云转到了一个W''系，目的是为了将当前帧的地图点与参考回环帧的地图点距离拉近   原因：可能是距离过远icp会失效？
        // pose_diff_t 可理解为tW''W
        ...
        {
            // get gtsam pose
            // TW'Lk = TW'W''*TW''W*TWLk 将当前帧点云经过icp与回环候选帧对齐（这样对齐的结果就是当前帧和回环帧的地图点基本重合） 旋转不变性
            ...
            // TW'Lk.inverse() * TWLh 将当前帧点云经过icp旋转得到的与回环候选帧对齐的位姿 与 和回环候选帧的位姿做差   在优化的时候，这个约束边的测量值为0
    ...
    void performLoopClosureDetection()
    {
        ...
        //找到20m内与当前时间戳相差30s以上的最近的关键帧作为 回环候选关键帧
        {
    ...
    {        
        ...
        if (cloudInfo.odomAvailable == true && cloudInfo.odomResetId == odomResetId)
        {
            ...
            else 
            {
                ...
                Eigen::Affine3f transBack = pcl::getTransformation(cloudInfo.odomX,    cloudInfo.odomY,     cloudInfo.odomZ,    //TwL1
                                                                   cloudInfo.odomRoll, cloudInfo.odomPitch, cloudInfo.odomYaw);
                // 获取VINS里程计相对于上一次的里程计增量transIncre
                // Tvins_k-1 * Tvins_k = T_vins_k-1_k   
                Eigen::Affine3f transIncre = lastVinsTransformation.inverse() * transBack;    //TL0L1 = TwL0.inverse() * TwL1         这里0：上一帧时刻   1：当前帧时刻

                // 将上一帧优化结束获得的关键帧估计结果transformTobeMapped转换为Eigen::Affine3f格式的transTobe
                Eigen::Affine3f transTobe = trans2Affine3f(transformTobeMapped);
                // Twlk^ = Twlk-1 * Tlk-1lk                Twl1(当前帧的迭代初值) = TwL0(上一帧优化后的值) * TL0L1(通过VINS里程计得到的两帧之间的位姿差)
    ...
    void cornerOptimization()
    {
        ...
        */
        updatePointAssociateToMap();   //Twl(预测值)
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                if (planeValid)
                {
                    ...
                    // 残差越大，权重越小
                    // 距离世界系原点越远，权重越大。越远的点位姿估计较不准确，因此将雷达系下的点转到世界系下的误差越大，此时得到的距离残差不太准确，这里的分母用距离来表示
                    //可以稀释这种误差
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        if (iterCount == 0) 
        {
            ...
            // 求解JT[Σ^(-1)]J的特征值填入matE，特征向量填入matV
    ...
    bool saveFrame()
    {
        ...
        // 计算最新关键帧的位姿  Twl0(上一关键帧isam后的位姿)
        Eigen::Affine3f transStart = pclPointToAffine3f(cloudKeyPoses6D->back());
        // 计算目前帧优化后的位姿 Twl1(当前帧scan2map后的位姿)
        Eigen::Affine3f transFinal = pcl::getTransformation(transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5], 
                                                            transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
        // 两帧之间的位姿变换 Eigen::Affine3f格式 Tl0l1
    ...
    void addOdomFactor()
    {
        ...
        {
            // 设置先验噪声 yaw和位置方差设置较大
            // Diagonal类继承自Gaussian类  这里roll、pitch的噪声设置较小，说明比较信任imu的roll、pitch初值，而yaw、x、y、z的噪声设置较大，表示对这四个变量的初值信任较低
            //这里对噪声的设置，主要考虑到其x、y、z只是相对位置，如果有GPS因子加入，提供了绝对位置，则优化的时候，xyz需要发生较大的变化
            noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-2, 1e-2, M_PI*M_PI, 1e8, 1e8, 1e8).finished()); // rad*rad, meter*meter
            // gtsam::NonlinearFactorGraph
            gtSAMgraph.add(PriorFactor<Pose3>(0, trans2gtsamPose(transformTobeMapped), priorNoise));
            // gtsam::Values
            initialEstimate.insert(0, trans2gtsamPose(transformTobeMapped));//先验的预测值和观测值都为同一个值，这里的transformTobeMapped中roll、pitch、yaw（或为0）
            //                                                                                                                                      由九轴imu直接提供，xyz设置为0
        }
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        // 更新了globalPath
        // 遇到关键帧后进行全局位姿图优化，之后对globalPath进行更新？我看这里只有添加新的位姿点，在哪里更新之前的位姿呢？
        //还是说由于在没有回环的情况下，新加入的约束边只是约束了当前帧与上一帧位姿，并未对前面的位姿造成影响？我觉得不应该
        //因为加入里程计因子，即使只优化前后两帧，当上一帧的位姿改变，自然会影响前面的位姿。因此这里存疑？
