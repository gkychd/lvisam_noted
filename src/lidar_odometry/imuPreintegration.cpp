...
public:
    ...
    // imu-lidar位姿变换
    gtsam::Pose3 imu2Lidar = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(-extTrans.x(), -extTrans.y(), -extTrans.z()));//Rot的四个参数依次是四元数中的w、x、y、z
    gtsam::Pose3 lidar2Imu = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(extTrans.x(), extTrans.y(), extTrans.z()));;//这里的imu2lidar和lidar2imu都为单位阵
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imuMsg)
    {
        ...
        // imu预积分器添加一帧imu数据，这个预积分的起始时刻是上一帧激光里程计时刻                以下都是lidar系下的加速度和角速度
        imuIntegratorImu_->integrateMeasurement(gtsam::Vector3(thisImu.linear_acceleration.x, thisImu.linear_acceleration.y, thisImu.linear_acceleration.z),
                                                gtsam::Vector3(thisImu.angular_velocity.x,    thisImu.angular_velocity.y,    thisImu.angular_velocity.z), dt);
        ...
        // 变换到lidar坐标系
        gtsam::Pose3 imuPose = gtsam::Pose3(currentState.quaternion(), currentState.position());//这里的imuPose实际上是雷达中心的位姿 Twl
        gtsam::Pose3 lidarPose = imuPose.compose(imu2Lidar); // Twl' = Twl * Tll'  转化为新的lidar位姿 -----> 但是这里imu2Lidar是单位矩阵，因此无变化
